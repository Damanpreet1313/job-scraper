...
REMOTE_PATTERNS = [
    re.compile(r"\b(remote|work from home|wfh|distributed|anywhere|worldwide|global)\b", re.IGNORECASE),
]
ONSITE_PATTERNS = [
...
def detect_work_type(location: str, description: str = "") -> str:
    ...
    if _has_pattern(haystack, REMOTE_PATTERNS):
        return "remote"
    if _has_pattern(haystack, HYBRID_PATTERNS):
        return "hybrid"
    if _has_pattern(haystack, ONSITE_PATTERNS):
        return "onsite"
...
def is_location_allowed(location: str, description: str = "") -> tuple[bool, str]:
    ...
    # Check if description mentions remote explicitly even if location is a city
    haystack = f"{location} {description}".lower()
    if _has_pattern(haystack, REMOTE_PATTERNS):
        return True, f"Remote mentioned in description for {location}"
